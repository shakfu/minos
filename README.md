...
One process. A goroutine per connection, an in-process fan-out, and SQLite for
the timeline.
...
A backfill is capped at the tail (`HistoryLimit`, 200 messages) and that cap is
...
- Retention beyond the transient room is specified and not built: archival, and the admin's read of it. Both are in `chat-concepts.md` under *Later*, along with what the core does to avoid foreclosing them.
